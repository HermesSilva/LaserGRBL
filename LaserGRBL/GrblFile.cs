...
namespace LaserGRBL
{
	public class GrblFile : IEnumerable<GrblCommand>
	{
		...
		public event OnFileLoadedDlg OnFileChanged; //same file, changed content: refresh drawing without reframing the view
		...
		/// <summary>Held while the command list is read (drawing) or rewritten (svg power/size change)</summary>
		public readonly object CommandsLock = new object();

		//SVG job power: vector import emits a constant S value for the whole job, so power
		//can be rescaled after loading without regenerating gcode from the source file.
		private bool mIsSvgSource = false;
		private string mLoadedFileName = null;
		private List<SvgSourceLine> mSvgBaseline = null;
		private bool mSvgHasFeed = false;
		private decimal mSvgOriginalMaxS = 0;
		private int mSvgPower = 0;
		private int mSvgScalePerc = 0;
		private int mSvgSpeedVal = SVG_SPEED_MAX;
		private int mSvgTargetPower = 0;
		private int mSvgTargetScale = 0;
		private int mSvgTargetSpeed = SVG_SPEED_MAX;
		private Thread mSvgTransformThread = null;
		private readonly object mSvgTransformLock = new object();

		//original gcode line as generated by the svg import, used as baseline for power and size changes
		private class SvgSourceLine
		{
			public readonly int Index;
			public readonly string Line;
			public readonly decimal S;
			public readonly bool SDecimal;
			public readonly bool HasGeometry;
			public readonly bool HasFeed;

			public SvgSourceLine(int index, string line, decimal s, bool sdecimal, bool hasgeometry, bool hasfeed)
			{
				Index = index;
				Line = line;
				S = s;
		...
		private void SafeLoadFile(ThreadStart loadFileAction, bool showInUseMessage = true)
        {
            if (CheckInUse(showInUseMessage)) return;
		...
		public void LoadImportedSVG(string filename, bool append, GrblCore core, ColorFilter filter)
        {
            SafeLoadFile(() =>
            {
				...
				//power slider is available only on a "pure" svg job (append mixes gcode of different origin)
				if (append)
					ClearSvgPowerInfo(filename);
				else
					BuildSvgPowerInfo(filename);
		...
		#region SVG job power and scale

		private static readonly System.Text.RegularExpressions.Regex SValueRegEx = new System.Text.RegularExpressions.Regex(@"S\d+(?:\.\d+)?", System.Text.RegularExpressions.RegexOptions.Compiled);
		private static readonly System.Text.RegularExpressions.Regex GeometryRegEx = new System.Text.RegularExpressions.Regex(@"(?<letter>[XYIJR])(?<value>-?\d+(?:\.\d+)?)", System.Text.RegularExpressions.RegexOptions.Compiled);
		private static readonly System.Text.RegularExpressions.Regex FValueRegEx = new System.Text.RegularExpressions.Regex(@"F\d+(?:\.\d+)?", System.Text.RegularExpressions.RegexOptions.Compiled);

		/// <summary>Scale can go from -99% (almost zero) to +900% (ten times bigger)</summary>
		public const int SVG_SCALE_MIN = -99;
		public const int SVG_SCALE_MAX = 900;

		/// <summary>Speed slider: +1 keeps the programmed feed, -300 slows it down to one third, linear in between</summary>
		public const int SVG_SPEED_MIN = -300;
		public const int SVG_SPEED_MAX = 1;

		/// <summary>True if the loaded file comes from a vector svg import and its power can be rescaled</summary>
		public bool CanChangeSvgLaserPower
		{ get { return mIsSvgSource && mSvgBaseline != null && mSvgOriginalMaxS > 0; } }

		/// <summary>True if the loaded file comes from a vector svg import and its size can be rescaled</summary>
		public bool CanChangeSvgScale
		{ get { return mIsSvgSource && mSvgBaseline != null && mSvgBaseline.Count > 0; } }

		/// <summary>True if the loaded file comes from a vector svg import and its feed rate can be rescaled</summary>
		public bool CanChangeSvgSpeed
		{ get { return mIsSvgSource && mSvgBaseline != null && mSvgHasFeed; } }

		/// <summary>Laser power (S value) currently applied to the whole svg job</summary>
		public int SvgLaserPower
		{ get { return mSvgPower; } }

		/// <summary>Size variation applied to the whole svg job, in percent (0 = original size)</summary>
		public int SvgScalePercent
		{ get { return mSvgScalePerc; } }

		/// <summary>Speed slider value applied to the whole svg job (+1 = programmed feed, -300 = one third)</summary>
		public int SvgSpeedValue
		{ get { return mSvgSpeedVal; } }

		/// <summary>Feed rate multiplier matching the given slider value: linear from 1 (at +1) to 1/3 (at -300)</summary>
		public static decimal SvgSpeedFactor(int value)
		{
			value = Math.Max(SVG_SPEED_MIN, Math.Min(SVG_SPEED_MAX, value));
			return 1m / 3m + (2m / 3m) * (value - SVG_SPEED_MIN) / (SVG_SPEED_MAX - SVG_SPEED_MIN);
		}

		/// <summary>True while a requested power/size/speed change has still to be applied: the UI must not realign its controls</summary>
		public bool SvgTransformPending
		{ get { return mSvgTargetPower != mSvgPower || mSvgTargetScale != mSvgScalePerc || mSvgTargetSpeed != mSvgSpeedVal; } }

		private void ClearSvgPowerInfo(string filename)
		{
			...
			mSvgHasFeed = false;
			mSvgOriginalMaxS = 0;
			mSvgPower = mSvgTargetPower = 0;
			mSvgScalePerc = mSvgTargetScale = 0;
			mSvgSpeedVal = mSvgTargetSpeed = SVG_SPEED_MAX;
		}

		//keep the gcode as generated by the svg import: power and size are always recomputed from this baseline,
		//so repeated changes do not accumulate rounding errors
		private void BuildSvgPowerInfo(string filename)
		{
			mLoadedFileName = filename;
			mIsSvgSource = true;
			mSvgBaseline = new List<SvgSourceLine>();
			mSvgHasFeed = false;
			mSvgOriginalMaxS = 0;
			mSvgScalePerc = mSvgTargetScale = 0;
			mSvgSpeedVal = mSvgTargetSpeed = SVG_SPEED_MAX;
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				cmd.BuildHelper();
				GrblCommand.Element s = cmd.S;
				string line = cmd.Command;
				cmd.DeleteHelper();

				decimal snumber = s != null ? s.Number : 0; //S0 is laser off: must stay zero
				bool hasgeometry = GeometryRegEx.IsMatch(line);
				bool hasfeed = FValueRegEx.IsMatch(line);

				if (snumber > 0 || hasgeometry || hasfeed)
				{
					System.Text.RegularExpressions.Match sm = SValueRegEx.Match(line); //smoothieware uses fractional S values
					mSvgBaseline.Add(new SvgSourceLine(i, line, snumber, sm.Success && sm.Value.IndexOf('.') >= 0, hasgeometry, hasfeed));

					if (snumber > mSvgOriginalMaxS)
						mSvgOriginalMaxS = snumber;

					mSvgHasFeed |= hasfeed;
				}
			}

			mSvgPower = mSvgTargetPower = (int)Math.Round(mSvgOriginalMaxS);
		}

		/// <summary>Rescale the S value of the whole loaded svg job, keeping the original proportions.
		/// Returns false only if the loaded file is not a svg job.</summary>
		public bool SetSvgLaserPower(int power)
		{
			if (!CanChangeSvgLaserPower)
				return false;

			return RequestSvgTransform(power, mSvgTargetScale, mSvgTargetSpeed);
		}

		/// <summary>Resize the whole loaded svg job, in percent of the original size (0 = original).
		/// Returns false only if the loaded file is not a svg job.</summary>
		public bool SetSvgScalePercent(int percent)
		{
			if (!CanChangeSvgScale)
				return false;

			return RequestSvgTransform(mSvgTargetPower, Math.Max(SVG_SCALE_MIN, Math.Min(SVG_SCALE_MAX, percent)), mSvgTargetSpeed);
		}

		/// <summary>Change the feed rate of the whole loaded svg job (+1 = programmed feed, -300 = one third).
		/// Returns false only if the loaded file is not a svg job.</summary>
		public bool SetSvgSpeedValue(int value)
		{
			if (!CanChangeSvgSpeed)
				return false;

			return RequestSvgTransform(mSvgTargetPower, mSvgTargetScale, Math.Max(SVG_SPEED_MIN, Math.Min(SVG_SPEED_MAX, value)));
		}

		//requests are coalesced: while a rewrite is running (or the drawing thread holds the command list)
		//only the last requested value survives, so dragging the slider or holding the spinner stays responsive
		private bool RequestSvgTransform(int power, int scaleperc, int speedval)
		{
			lock (mSvgTransformLock)
			{
				mSvgTargetPower = power;
				mSvgTargetScale = scaleperc;
				mSvgTargetSpeed = speedval;

				if (mSvgTransformThread == null)
				{
					mSvgTransformThread = new Thread(SvgTransformWorker);
					mSvgTransformThread.Name = "SVG Transform Thread";
					mSvgTransformThread.Start();
				}
			}
		...
		private void SvgTransformWorker()
		{
			try
			{
				while (true)
				{
					int power, scaleperc, speedval;

					lock (mSvgTransformLock)
					{
						power = mSvgTargetPower;
						scaleperc = mSvgTargetScale;
						speedval = mSvgTargetSpeed;

						if (mSvgBaseline == null || (power == mSvgPower && scaleperc == mSvgScalePerc && speedval == mSvgSpeedVal))
						{
							mSvgTransformThread = null; //nothing more to do
							return;
						}
					}

					if (!WaitLoadingEnd())
						continue; //a file load is running: loop back and pick up the latest requested value

					long start = Tools.HiResTimer.TotalMilliseconds;

					mLoadingThread = Thread.CurrentThread; //keep the file "in use" while rewriting the command list
					try { RewriteSvgLines(power, scaleperc, speedval); }
					finally { mLoadingThread = null; }

					mSvgPower = power;
					mSvgScalePerc = scaleperc;
					mSvgSpeedVal = speedval;

					long elapsed = Tools.HiResTimer.TotalMilliseconds - start;
					RiseOnFileChanged(mLoadedFileName, elapsed); //redraw preview and estimated time, keeping current zoom/pan
				}
			}
			catch (Exception ex)
			{
				Logger.LogException("SvgTransform", ex);
				lock (mSvgTransformLock) mSvgTransformThread = null;
			}
		}

		//a file load replaces the whole list (and the baseline): never rewrite while it is running
		//(the drawing thread does not need to be waited for: it shares CommandsLock)
		private bool WaitLoadingEnd()
		{
			for (int i = 0; i < 200; i++) //up to 2 seconds, then loop back to refresh the target value
			{
		...
		private void RewriteSvgLines(int power, int scaleperc, int speedval)
		{
			decimal factor = 1 + scaleperc / 100.0m;
			decimal speedfactor = SvgSpeedFactor(speedval);

			lock (CommandsLock) //do not swap commands while the drawing thread is walking the list
			{
				foreach (SvgSourceLine ssl in mSvgBaseline)
				{
					string newline = ssl.Line;

					if (ssl.S > 0 && mSvgOriginalMaxS > 0)
					{
						decimal newS = ssl.S * power / mSvgOriginalMaxS;
						string svalue = ssl.SDecimal
							? newS.ToString("0.00", System.Globalization.CultureInfo.InvariantCulture)
							: Math.Round(newS).ToString(System.Globalization.CultureInfo.InvariantCulture);

						newline = SValueRegEx.Replace(newline, "S" + svalue);
					}

					if (ssl.HasGeometry && factor != 1)
					{
						newline = GeometryRegEx.Replace(newline, m =>
						{
							decimal value = decimal.Parse(m.Groups["value"].Value, System.Globalization.CultureInfo.InvariantCulture);
							return m.Groups["letter"].Value + (value * factor).ToString("0.###", System.Globalization.CultureInfo.InvariantCulture);
						});
					}

					if (ssl.HasFeed && speedfactor != 1)
					{
						newline = FValueRegEx.Replace(newline, m =>
						{
							decimal value = decimal.Parse(m.Value.Substring(1), System.Globalization.CultureInfo.InvariantCulture);
							return "F" + (value * speedfactor).ToString("0.###", System.Globalization.CultureInfo.InvariantCulture);
						});
					}

					if (newline != list[ssl.Index].Command)
					{
						GrblCommand old = list[ssl.Index];
						list[ssl.Index] = new GrblCommand(newline);
						old.Dispose();
					}
				}
		...
		private void RiseOnFileChanged(string filename, long elapsed)
		{
			if (OnFileChanged != null)
				OnFileChanged(elapsed, filename);
		}
		...
		internal void DrawOnGraphics(Graphics g, Size size)
		{
			...
			lock (CommandsLock) //the list can be rewritten while drawing (svg power/size change)
